#!/bin/bash
################################################################################
# approach_b_topmed.sh
#
# APPROACH B: Minimal QC-Before + TOPMed (Southam 2011 Recommendation)
#
...
INPUT_PLINK=""
OUTPUT_DIR="./results/approach_b_topmed"
TOPMED_TOKEN=""
TOPMED_PASSWORD=""
THREADS=4

# Minimal QC thresholds (Southam 2011 style)
GENO_THRESHOLD=0.10    # Lenient 90% variant call rate
MIND_THRESHOLD=0.10    # Lenient 90% sample call rate
...
APPROACH B: Minimal QC-before-imputation (Southam 2011) + TOPMed.

Based on Southam et al. 2011 (Eur J Hum Genet) which found that
stringent pre-imputation QC can REDUCE imputation quality.
...
Optional:
  ...
  --topmed-password PASS   TOPMed password for decryption
...
if [[ -z "$INPUT_PLINK" ]] || [[ -z "$TOPMED_TOKEN" ]]; then
    echo "ERROR: --input and --topmed-token are required"
    print_usage
    exit 1
fi
...
log "=============================================="
log "APPROACH B: Minimal QC + TOPMed (Southam 2011)"
log "=============================================="
...
# =============================================================================
# STEP 1: MINIMAL PRE-IMPUTATION QC (Southam 2011)
# =============================================================================

log ""
log "=== STEP 1: Minimal Pre-Imputation QC (Southam 2011) ==="
...
log ""
log "=== STEP 3: Submit to TOPMed Server ==="
...
if [[ -d "../imputation/topmed_results" ]]; then
    ...
    # Convert to PLINK for further QC
...
# =============================================================================
# STEP 5: THOROUGH POST-IMPUTATION QC (Key from Southam/Charon)
# =============================================================================
...
if [[ -f "../qc_after/imputed_r2filt.bed" ]]; then
    run_post_imputation_qc "../qc_after/imputed_r2filt" "approach_b_topmed" ${THREADS} \
        ${MAF_THRESHOLD} ${HWE_PVALUE} 0.02 0.02

    log "  Final: $(count_variants_samples approach_b_topmed)"
fi
...
log ""
log "=============================================="
log "APPROACH B (TOPMed) Complete!"
log "=============================================="
log "Total time: $(seconds_to_human ${TOTAL_TIME})"
log ""
log "Key differences from Approach A:"
log "  - Minimal QC before imputation (Southam 2011 recommendation)"
log "  - Thorough QC AFTER imputation instead"
log ""
