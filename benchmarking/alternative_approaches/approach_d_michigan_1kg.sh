...
#
# APPROACH D: Merge AFTER Imputation with Traditional R² Filter + Michigan 1KG
#
...
APPROACH D: Merge AFTER Imputation with Traditional R² Filter + Michigan 1000G
...
if [[ -f "../qc_after/imputed_r2filt.bed" ]]; then
    ...
    run_thorough_qc "../qc_after/imputed_r2filt" "approach_d_michigan_1kg" ${THREADS}
    ...
    # GWAS track: MAF > 1%
    plink2 --bfile approach_d_michigan_1kg \
        --maf 0.01 \
        --make-bed \
        --out approach_d_michigan_1kg_gwas \
        --threads ${THREADS}

    log "    GWAS track (MAF>1%): $(count_variants_samples approach_d_michigan_1kg_gwas)"

    # RVAS track: All variants (no MAF filter)
    cp approach_d_michigan_1kg.* approach_d_michigan_1kg_rvas.*
    log "    RVAS track (all variants): $(count_variants_samples approach_d_michigan_1kg_rvas)"
fi
...
log ""
log "Output tracks:"
log "  GWAS: ${OUTPUT_DIR}/final/approach_d_michigan_1kg_gwas"
log "  RVAS: ${OUTPUT_DIR}/final/approach_d_michigan_1kg_rvas"
